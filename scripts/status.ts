...
async function main() {
  const counts: [string, number][] = [
    ...
    ['  of which cars', await prisma.vehicle.count({ where: { category: 'car' } })],
    ['  of which RVs', await prisma.vehicle.count({ where: { category: 'rv' } })],
    ['  of which motorcycles', await prisma.vehicle.count({ where: { category: 'motorcycle' } })],
    ['  with complaints loaded', await prisma.vehicle.count({ where: { complaintsLoadedAt: { not: null } } })],
    ['  with NCAP rating', await prisma.vehicle.count({ where: { overallRating: { not: null } } })],
    ...
    ['  with snow dates', await prisma.climateData.count({ where: { firstSnowDate: { not: null } } })],
    ['  snow rare', await prisma.climateData.count({ where: { snowSeasons: { not: null }, firstSnowDate: null } })],
